 ...
 */
async function confirmDefaultNo(question) {
 ...
 */
function makeBirthdayEvent(title, day, reminderMinutes) {
...
function reportCreatedBirthday(created, open) {
    ...
    if (open && created.htmlLink)
        openUrl(created.htmlLink);
}
/** Options that a birthday event cannot carry; -birthday with any of these is an error. */
function rejectBirthdayConflicts(parsed) {
    const bad = [];
    if (parsed.rrule)
        bad.push('-rrule (birthdays always repeat yearly)');
    if (parsed.transparency === 'opaque')
        bad.push('-busy (Google shows birthdays as free)');
...
function checkIcsAssoc() {
    ...
    try {
        ...
        // launcher exe.
        const out = execSync(`reg query "HKCU\\Software\\Classes\\${ICS_PROGID}\\shell\\open\\command" /ve`, { encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'] });
    ...
    switch (parsed.command) {
        ...
        case 'add': {
            ...
            if (parsed.args.length >= 2 && !parsed.clip) {
                ...
                const token = await getAccessToken(user, true);
                if (!parsed.allDay) {
                    const conflicts = await checkProximity(token, parsed.calendar, new Date(event.start.dateTime), new Date(event.end.dateTime));
            ...
            for (const extracted of extractedEvents) {
                ...
                const conflicts = await checkProximity(token, parsed.calendar, zonedWallClockToDate(startDt, tz), zonedWallClockToDate(endDt, tz));
                if (event.transparency === 'opaque')
                    busyConflicts += conflicts;
            }
            if (events.length === 0) {
